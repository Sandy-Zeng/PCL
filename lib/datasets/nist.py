...
class nist(imdb):
  def __init__(self, image_set, year, dist_path=None):
    ...
    self._data_path= '/data/NIST2016/probe_save'
  ...
  def mask_path_from_index(self, index):
    ...
    """
    for ext in self._image_ext:
      mask_path = os.path.join('/data/NIST2016/mask', index + ext)
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    for i, cls in enumerate(self._classes):
      ...
      print(prec)
      aps += [ap]
      print(('AP for {} = {:.4f}, AP 50-95 for {} = {:.4f}, recall = {:.4f}, precision = {:.4f}'.format(cls, ap, cls,
                                                                                                        AP50_95,
                                                                                                        rec[-1],
                                                                                                        prec[-1])))

      # rec, prec, ap = voc_eval(
      #   filename, filename2, annopath, imagesetfile, cls_ind, cachedir, ovthresh=0.3,
      #   use_07_metric=use_07_metric, fuse=False)
      # print(rec)
      # print(prec)
