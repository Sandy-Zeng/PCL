...
class coco(imdb):
  ...
  def image_path_from_index(self, index):
    ...
    for ext in self._image_ext:
      image_path = index + ext
      image_path1 = os.path.join('/data/coco_synthetic/train_data', index + ext)
      image_path2=os.path.join('/data/coco_synthetic/train2014', index + ext)
      if os.path.isfile(image_path):
        return image_path
      elif os.path.isfile(image_path1):
        return image_path1
      elif os.path.isfile(image_path2):
        return image_path2
      else:
        continue
  ...
  def _load_image_set_index(self):
    ...
    image_set_file = os.path.join('/data/coco_synthetic', self._image_set + '.txt')
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    for i, cls in enumerate(self._classes):
      ...
      with open(os.path.join(output_dir, cls + '_res.txt'), 'w') as f:
        f.write(('AP for {} = {:.4f}, AP50-95 for {} = {:.4f}, recall = {:.4f}, precision = {:.4f}, F1 score = {:.4f}'.format(cls, ap, cls, AP50_95, rec[-1],
                                                                                                    prec[-1],
                                                                                                    F1_score)))
