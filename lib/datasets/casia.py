...
class casia(imdb):
  ...
  def mask_path_from_index(self, index):
    ...
    for ext in self._image_ext:
      mask_path = os.path.join('/jizhicfs/yuyuanzeng/data/CASIA/CASIA1_GT/CM',
                              index + '_gt' + ext)
      mask_path1 = os.path.join('/jizhicfs/yuyuanzeng/data/CASIA/CASIA1_GT/Sp',
                               index + '_gt' + ext)
      mask_path2 = os.path.join('/jizhicfs/yuyuanzeng/data/CASIA/CASIA1_GT/mask2',
                               index + '_gt' + ext)
      mask_path3 = os.path.join('/jizhicfs/yuyuanzeng/data/CASIA/casia2groundtruth/CASIA2_GT',
                                index + '_gt' + ext)
      if os.path.isfile(mask_path):
        return mask_path
      elif os.path.isfile(mask_path1):
        return mask_path1
      elif os.path.isfile(mask_path2):
        return mask_path2
      elif os.path.isfile(mask_path3):
        return mask_path3
      else:
        continue
    src_image = index.split('_')[4]
    tar_image = index.split('_')[5]
    split_name = index.split('_')
    mask_name = split_name[0] + '_' + split_name[1] + '_' + split_name[2] + '_' + split_name[
      3] + '_' + tar_image + '_' + src_image + '_' + split_name[6] + '_gt'
    for ext in self._image_ext:
      mask_path = os.path.join('/jizhicfs/yuyuanzeng/data/CASIA/CASIA1_GT/CM', mask_name + ext)
      mask_path1 = os.path.join('/jizhicfs/yuyuanzeng/data/CASIA/CASIA1_GT/Sp', mask_name + ext)
      mask_path2 = os.path.join('/jizhicfs/yuyuanzeng/data/CASIA/CASIA1_GT/mask2', mask_name + ext)
      if os.path.isfile(mask_path):
        return mask_path
      elif os.path.isfile(mask_path1):
        return mask_path1
        ...
        continue
    assert os.path.isfile(mask_path) or os.path.isfile(mask_path1), \
         'Path does not exist: {}'.format(index)
    return 0
  ...
  def image_path_from_index(self, index):
    ...
    for ext in self._image_ext:
      #image_path = os.path.join('/home-3/pengzhou@umd.edu/work/xintong/medifor/portrait/test_data',
                              #index + ext)
      image_path = os.path.join(self._data_path, index + ext)
      image_path1 = index + ext
  ...
  def _load_image_set_index(self):
    ...
    image_set_file = os.path.join('/mnt/wx_feature/home/yuyuanzeng/PS/RGBN_SSL/data/casia',
                                  self._image_set + '.txt')
  ...
  def roidb_gt(self,image_id):
    ...
    for ix in range(num_objs):
      ...
      if y1<0:
        y1=0 
      try:
        cls_name = image_id.split(' ')[ix*5+5]
        if cls_name == 'semi':
          cls = -1
        else:
  ...
  def _get_voc_noise_results_file_template(self, output_dir):
    # VOCdevkit/results/VOC2007/Main/<comp_id>_det_test_aeroplane.txt
  ...
  # def _write_voc_results_file(self, all_boxes, test_images, output_dir):
  ...
  #       for im_ind, _ in enumerate(range(len(all_boxes[cls_ind]))):
  #         # index = self._image_index[im_ind]
  #         index = test_images[im_ind]
  ...
  def _do_python_eval(self, output_dir='output'):
    ...
    for i, cls in enumerate(self._classes):
      if cls == '__background__' or cls == self.classes[0]:
        ...
        #continue
      filename = self._get_voc_results_file_template(output_dir).format(cls)
      filename2 = self._get_voc_noise_results_file_template(output_dir).format(cls)
      # print(cls_ind)
      ...
      for k in range(10):
        rec, prec, ap = voc_eval(
          filename, filename2, annopath, imagesetfile, cls_ind, cachedir, ovthresh=iou,
          use_07_metric=use_07_metric, fuse=False)
        iou += 0.05
        print('iou %.2f ap %.4f' % (iou, ap))
        AP50_95 += ap
      AP50_95 /= 10.0
      ...
      rec, prec, ap = voc_eval(
        ...
        use_07_metric=use_07_metric,fuse=False)
      aps += [ap]
      print(('AP for {} = {:.4f}, AP 50-95 for {} = {:.4f}, recall = {:.4f}, precision = {:.4f}'.format(cls, ap, cls, AP50_95, rec[-1],prec[-1])))
      F1_score = (2 * prec[-1] * rec[-1]) / (prec[-1] + rec[-1] + 1e-9)
      print('F1 Score:', F1_score)
      print('AP 50-95', AP50_95)
  ...
  # def evaluate_detections(self, all_boxes, test_images, output_dir):
  #   self._write_voc_results_file(all_boxes, test_images, output_dir)
  #   ap, map = self._do_python_eval(output_dir)
