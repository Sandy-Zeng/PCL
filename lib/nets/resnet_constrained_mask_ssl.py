...
from nets.network_constrained_mask_ssl import Network
...
class resnet_constrained_mask_ssl(Network):
  ...
  def build_network(self, sess, is_training=True):
    ...
    if cfg.RESNET.FIXED_BLOCKS == 3:
        with slim.arg_scope(resnet_arg_scope(is_training=False)):
            ...
            net_conv3, _ = resnet_v1.resnet_v1(net,
                                               blocks[0:cfg.RESNET.FIXED_BLOCKS - 1],
                                               global_pool=False,
    ...
    # C_4 =tf.Print(C_4,[tf.shape(C_4)],message='Value of %s' % 'C_4', summarize=4, first_n=1)
    # net_conv4=tf.Print(net_conv4,[tf.shape(net_conv4)],message='Value of %s' % 'net_conv4', summarize=4, first_n=1)
    ...
    with tf.variable_scope('mask_prediction'):
      with tf.variable_scope('feature_fuse', 'feature_fuse'):
          ...
          # mask_fuse = C_3 * 0.5 + net_conv3 * 0.5
          # feature_fuse = net_conv4
          # feature_fuse = C_4
          feature_fuse = C_4 * 0.5 + net_conv4 * 0.5
          # mask_fuse = tf.concat(3, [C_4, net_conv4]) 
          
          # feature_fuse = slim.conv2d(mask_fuse, 1024, [1, 1], padding='VALID', trainable=is_training, weights_initializer=initializer, scope='mask_fuse')
      # if is_training:
        # mask_box, indices = self._proposal_mask_train_layer(rois, 'mask_proposal')
      # else:  
