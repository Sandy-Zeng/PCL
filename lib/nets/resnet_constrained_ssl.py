...
from nets.network_constrained_ssl import Network
...
class resnet_constrained_ssl(Network):
  ...
  def build_network(self, sess, is_training=True):
    ...
    if cfg.RESNET.FIXED_BLOCKS == 3:
        with slim.arg_scope(resnet_arg_scope(is_training=False)):
            net = self.build_base(self._image, self._resnet_scope)
    ...
    # channel_attention, spatial_attention = self.cbam_module(inputs=C_4, name="rpn_conv1")
    # net_conv4 = net_conv4 * channel_attention * spatial_attention

    with tf.variable_scope(self._resnet_scope, self._resnet_scope):
      # build the anchors for the image
      self._anchor_component()
      rpn1 = slim.conv2d(net_conv4, 512, [3, 3], trainable=is_training, weights_initializer=initializer,
                        scope="rpn_conv/3x3")
    ...
    with slim.arg_scope(resnet_arg_scope(is_training=is_training)):
        noise_fc7, end_point1 = resnet_v1.resnet_v1(noise_pool5,
                                   blocks[-1:],
                                   global_pool=False,
                                   include_root_block=False,
                                   scope='noise')
    ...
    rgb_feat = tf.reshape(slim.avg_pool2d(fc7, [7, 7]), [-1, 2048])
    noise_feat = tf.reshape(slim.avg_pool2d(noise_fc7, [7, 7]), [-1, 2048])

    # rgb_feat = slim.fully_connected(rgb_feat, 2048, weights_initializer=initializer,
    #                                    trainable=is_training, activation_fn=tf.nn.relu, scope='rgb_mapping')
    # rgb_feat = slim.fully_connected(rgb_feat, 128, weights_initializer=initializer,
    #                                 trainable=is_training, activation_fn=None, scope='rgb_mlp')
    # noise_feat = slim.fully_connected(noise_feat, 2048, weights_initializer=initializer,
    #                                 trainable=is_training, activation_fn=tf.nn.relu, scope='noise_mapping')
    # noise_feat = slim.fully_connected(noise_feat, 128, weights_initializer=initializer,
    #                                 trainable=is_training, activation_fn=None, scope='noise_mlp')
    
    rgb_feat = slim.fully_connected(rgb_feat, 2048, weights_initializer=tf.contrib.layers.xavier_initializer(),
                                       trainable=is_training, activation_fn=None, scope='rgb_mapping')
    rgb_feat = slim.batch_norm(rgb_feat, activation_fn=None)
    rgb_feat = tf.nn.relu(rgb_feat)
    rgb_feat = slim.fully_connected(rgb_feat, 128, weights_initializer=tf.contrib.layers.xavier_initializer(),
                                    trainable=is_training, activation_fn=None, scope='rgb_mlp')


    noise_feat = slim.fully_connected(noise_feat, 2048, weights_initializer=tf.contrib.layers.xavier_initializer(),
                                    trainable=is_training, activation_fn=None, scope='noise_mapping')
    noise_feat = slim.batch_norm(noise_feat, activation_fn=None)
    noise_feat = tf.nn.relu(noise_feat)
    noise_feat = slim.fully_connected(noise_feat, 128, weights_initializer=tf.contrib.layers.xavier_initializer(),
                                    trainable=is_training, activation_fn=None, scope='noise_mlp')
    ...
    #   noise_cls_score = slim.fully_connected(cls_fc7, self._num_classes, weights_initializer=initializer,
    #                                    trainable=is_training, activation_fn=None, scope='cls_score')
    #   cls_prob = self._softmax_layer(noise_cls_score, "cls_prob")
    ...
    self._contrastive_anchor_target_layer(cls_prob[:, 1],  fc7, "cls_anchor")
    self._contrastive_proposal_target_layer(bbox_pred[:, 4:8], fc7, rois, "bbox_anchor")
    ...
    self._predictions["rgb_feat"] = rgb_feat
    self._predictions['noise_feat'] = noise_feat
