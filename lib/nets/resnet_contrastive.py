...
from nets.network_contrastive import Network
...
class resnet_contrastive(Network):
  ...
  def build_network(self, sess, is_training=True):
    ...
    with tf.variable_scope(self._resnet_scope, self._resnet_scope):
      ...
      rpn_cls_prob = tf.Print(rpn_cls_prob, [tf.shape(rpn_cls_prob)], message='Value of %s' % 'rpn_cls_prob', summarize=4, first_n=1)
      if is_training:
        rois, roi_scores = self._proposal_layer(rpn_cls_prob, rpn_bbox_pred, "rois")
        rpn_labels = self._anchor_target_layer(rpn_cls_score, "anchor")
        rois = tf.Print(rois, [tf.shape(rois)], message='Value of %s' % 'roi', summarize=4, first_n=1)
    ...
    rgb_fc = fc7
    fc7 = tf.reshape(fc7, [-1, 7, 7, 2048])
    noise_fc7 = tf.reshape(noise_fc7, [-1, 7, 7, 2048])

    rgb_feat = tf.reshape(slim.avg_pool2d(rgb_fc, [7, 7]), [-1, 2048])
