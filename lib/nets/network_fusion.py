...
class Network(object):
  ...
  def get_ela_feature(self, image):
    ...
    temp_path = './test.jpg'
    im.save(temp_path, quality=90)
    temporary = Image.open(temp_path)

    diff = ImageChops.difference(im, temporary)
    extrema = diff.getextrema()
    max_diff = max([ex[1] for ex in extrema])
    scale = 255.0 / max_diff
    diff = ImageEnhance.Brightness(diff).enhance(scale)
    diff_mat = np.asarray(diff)
    os.remove(temp_path)
    return diff_mat

  def get_ela(self, data):
    train_data = data
    # print(train_data[0].shape)
    ela_feature = []
    for i in range(train_data.shape[0]):
      feat = self.get_ela_feature(train_data[i])
      feat = feat / 255.0
      ela_feature.append(np.expand_dims(feat, axis=0))
    ela_feature = np.concatenate(ela_feature, axis=0)
    # print(ela_feature.shape)
    return ela_feature
  ...
  def train_step(self, sess, blobs, train_op, iter):
    # ela_feature = self.get_ela(blobs['data'])
    # blobs['data'] = blobs['data'] * ela_feature
    feed_dict = {self._image: blobs['data'], self.noise:blobs['noise'], self._im_info: blobs['im_info'],
                 self._gt_boxes: blobs['gt_boxes']}
    rpn_cross_entropy,rpn_loss_box,cross_entropy,loss_box,loss, _ = sess.run([self._losses["rpn_cross_entropy"],
                                                                        ...
                                                                        train_op],
                                                                       feed_dict=feed_dict)
    return rpn_cross_entropy,rpn_loss_box, cross_entropy, loss_box, loss, 0
  ...
  def train_step_with_summary(self, sess, blobs, train_op, iter):
    ...
    rpn_cross_entropy,rpn_loss_box, cross_entropy,loss_box,loss, summary, _ = sess.run([self._losses["rpn_cross_entropy"],
                                                                                 ...
                                                                                 train_op],
                                                                                feed_dict=feed_dict)
    return rpn_cross_entropy,rpn_loss_box, cross_entropy,loss_box, loss, summary, 0
