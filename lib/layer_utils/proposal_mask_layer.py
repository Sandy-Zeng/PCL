...
def proposal_train_mask_layer(rois, bbox_target, labels, im_info, num_classes, training, testing):

  image_info=im_info[0]

  boxes = rois[:, 1:5]/image_info[2]

  bbox_target = np.reshape(bbox_target, [bbox_target.shape[0], -1])
  ...
  for ind in range(1,num_classes):
    if ind==0:
      ...
      cls_boxes = pred_boxes[:, 4 * ind:4 * (ind + 1)]
      cls_boxes = cls_boxes*image_info[2]
      dets = cls_boxes.astype(np.float32)
